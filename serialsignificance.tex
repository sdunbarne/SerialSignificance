...
% To Do:
% \begin{enumerate}
%     \item Add in some details about reversibility and the connection
%       to detailed balance.
%     \item
%         Find a more descriptive name for the outliers.
%     \item
%         Draw some figures to illustrate outliers.
%     \item
...
%             \item
%                 Random walk with reflective boundary
%             \item
...
% A simple heuristic used in practice is to sample ranks of states from
% long random trajectories on the Markov chain and compare these with the
% rank of the presented state; if the presented state is a \( 0.1\% \)
% outlier compared with the sampled ranks (its rank is in the bottom \(
% 0.1\% \) of sampled ranks), then this observation should correspond to a
% \( p \) value of \( 0.001 \).  This significance is not rigorous,
% however, without good bounds on the mixing time of the Markov chain.  [Need
% more explanation here about the stationary distribution.]
...
clear.
% classical tools like the \( t \)-test are often applied by making
% simplifying assumptions about the distribution in such cases.
...
\begin{enumerate}
    ...
    \item
        Make a sequence of random changes to the states according to the
...
\begin{remark}
    Let \( \tau_2 \) be the relaxation time%
    \index{relaxation time}
    for \( X \) defined as \( 1/ (1-\lambda_2) \) where \( \lambda_2 \)
    is the spectral gap for \( X \). The relaxation time is intimately
    related to the mixing time of \( X \).  [Maybe specify how, using
    the lower bound in the section on spectral decomposition.]
\end{remark}

% @article{10.1214/aoap/1028903453,
% author = {Pascal Lezaud},
...
% URL = {https://doi.org/10.1214/aoap/1028903453}
% }

% This is the probable reference for the Gillman Theorem
% [Gil98] D. Gillman. A Chernoff bound for random walks on expander graphs. SIAM J. Comput.,
% 27(4):1203 - 1220, 1998, https://doi.org/10.1137/S0097539794268765
% See also
% D. Gillman, Thesis (Ph. D.)--Massachusetts Institute of Technology,
% Dept. of Mathematics, 1993.
% http://hdl.handle.net/1721.1/17332
...
\begin{exercise}
    Given a Markov chain \( X \) with labels \( v:  \mathcal{X} \to
