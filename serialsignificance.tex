...
\begin{proof}
    \begin{enumerate}
        ...
        \item
            ...
            proof, use the \( \ell = 0 \) case, so the minimum value is
            the focus.
        % \item
        %     For \( 0 \le j \le n \) define
        %     \[
...
\begin{algorithm}[H]
    DontPrintSemicolon; \KwData{Markov chain \( X \), starting state \(
    \sigma_0 \), value function \( v(\cdot) \)} \KwResult{probability \(
...
\begin{algorithm}[H]
    DontPrintSemicoln \SetKwInOut{Input}{input} \SetKwInOut{Output}{output}
    \SetKwData{pL}{pathLength} \SetKwData{nT}{nTrials} \SetKwData{res}{results}
    \SetKwData{oT}{outlierThreshold} \SetKwData{nEO}{nEpsilonOutlier}
    \SetKwData{pEO}{probEpsOutlier} \SetKwFunction{Floor}{floor}
